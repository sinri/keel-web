---
title: Keel-Web 5.0.3 用户使用文档
---

# Keel-Web 5.0.3 用户使用文档

Keel-Web 是 Keel 的 Web 功能组件，基于 [Vert.x 5](https://vertx.io/) 构建，提供 HTTP 服务端、HTTP 客户端响应萃取、TCP 和 UDP 网络通信等能力。源码位于 [sinri/keel-web](https://github.com/sinri/keel-web)。
...
├── io.vertx.core                   (Vert.x Core 5.1.3)
...
- FastDocs 的目录缓存按实例隔离，静态资源从文档根目录提供，并加强路径校验。
- 请求体解析使用 Vert.x 的 MIME 解析结果；multipart 文件通过 `RoutingContext.fileUploads()` 读取。
- TCP/UDP 的 INFO 日志默认不再记录完整 payload；按需开启受长度限制的 DEBUG payload。
- 增加 FastDocs、响应萃取、请求体和路由注册的回归测试。
