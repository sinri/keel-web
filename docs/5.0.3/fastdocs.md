...
静态资源根目录与 Markdown 根目录一致，因此示例中的 `images/logo.png` 可通过 `/docs/images/logo.png` 访问。
...
同一进程中可以安装多个 FastDocs 实例；目录树与渲染结果按实例缓存，不会在不同文档根目录之间串用。标题和页脚应在开始处理请求前设置。
