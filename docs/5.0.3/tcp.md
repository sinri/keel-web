...
每个 Socket 实例自带日志记录器，主题为 `TcpSocket`，分类标签为 `socket_id:{socketID}`。INFO 默认只记录报文大小，不包含报文内容。

```java
getLogger().info(r -> r.message("Custom log").buffer(someBuffer)); // 仅摘要
```

排障时可在部署前显式开启 DEBUG payload。默认最多记录前 256 字节，超出部分会截断：

```java
wrapper.setDebugPayloadLoggingEnabled(true)
       .setDebugPayloadMaxBytes(512);
```

同时需要将对应 Logger 的可见级别配置为 DEBUG。payload 可能包含凭据或个人信息，只应短期开启。
...
netServer.connectHandler(socket -> {
    KeelBasicSocketWrapper wrapper = new KeelBasicSocketWrapper(socket);
    wrapper.setIncomingBufferProcessor(buffer ->
               wrapper.write("Received: " + buffer.toString()))
           .setCloseHandler(v -> System.out.println("Client disconnected"));
    wrapper.deployMe(keel);
});
