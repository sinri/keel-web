...
- 数据报接收事件（含发送方地址、端口和字节数）
- 数据报发送事件（含目标地址、端口和字节数）
- 异常事件
- 关闭事件

通过 `getLogger()` 可获取日志记录器进行自定义日志输出。

INFO 默认不记录数据报 payload。排障时可以显式启用受长度限制的 DEBUG payload：

```java
transceiver.setDebugPayloadLoggingEnabled(true)
           .setDebugPayloadMaxBytes(512);
```

默认采样上限为 256 字节；同时需要将对应 Logger 的可见级别配置为 DEBUG。payload 可能包含敏感信息，不建议长期启用。
