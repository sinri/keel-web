...
专为虚拟线程环境设计的请求接待类。它允许在虚拟线程中直接运行同步逻辑；阻塞操作仍应控制超时并避免无限等待。
...
public class GetUserReceptionist extends KeelWebFutureReceptionistInVirtualThread<JsonObject> {
    ...
    protected JsonObject handleInVirtualThread() throws KeelWebApiError {
        ...
        // 此方法运行在虚拟线程，可调用同步服务方法
        JsonObject user = queryUserSynchronously(userId);
...
用于读取 JSON (`application/json`) 和表单字段。使用前必须先由 Vert.x `BodyHandler` 处理请求；使用
`@ApiMeta` 时保持 `requestBodyNeeded = true`（默认值）即可。对于 `multipart/form-data`，本类只读取普通字段，上传文件应通过 `RoutingContext.fileUploads()` 获取。
